...
impl FromStr for Compartment {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut contents: Vec<Priority> = s.chars()
            .map(|c| c.try_into())
...
fn part1() {
    let prio = get_input("day_3.txt")
        .lines()
        .map(|s| s.parse::<Rucksack>().expect("Not a sack").get_duplicate().expect("No duplicates").0)
        .sum::<u32>();
    println!("{prio}")
}
...
impl Group {
    ...
    fn badge(self) -> Result<Priority, ()> {
        ...
        for p in first {
            if second.binary_search(&p).and(third.binary_search(&p)).is_ok() {
                return Ok(p);
            }
        }
